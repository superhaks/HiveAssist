...
def image_processing(img_file_buffer):
  ...
  #   cv2_img = cv2.imdecode(np.frombuffer(bytes_data, np.uint8), cv2.IMREAD_COLOR)
    ...
    faces = RetinaFace.detect_faces(img_file_buffer)
    objs = DeepFace.analyze(img_path=img_file_buffer, actions=['emotion'], detector_backend ="ssd",enforce_detection= False)
